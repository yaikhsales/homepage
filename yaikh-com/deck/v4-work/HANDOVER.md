...
`ASK=client node make-yai-deck-v4.js` → `yai-deck-v4-client.html` (also built by `all`). 3 slides: (1) same title slide as the pitch decks, (2) agent constellation — tabs + agent names only, no icons, mirrored in `CONSTELLATION` from `yaikh-dashboard/src/data/module.js`, (3) empty for now. Not on the website yet.
