...
export default function SubscribeClient({ initialPaidTran }: { initialPaidTran?: string }) {
  ...
  useEffect(() => {
    ...
    if (payState === "paid") { sessionStorage.removeItem(DRAFT_KEY); return; }
    sessionStorage.setItem(DRAFT_KEY, JSON.stringify({
      selectedPlan, companyName, country, contactName, contactEmail, agreed,
    }));
  }, [hydrated, selectedPlan, companyName, country, contactName, contactEmail, agreed, payState]);

  const plan = PLANS.find((p) => p.name === selectedPlan) || null;
  const amount = plan ? parsePrice(plan.price) : 0;
  const busy = payState === "opening" || payState === "waiting";
  ...
  const payWithAba = async (payment_option: "" | "abapay_khqr" | "cards" = "") => {
    if (!plan || amount <= 0) return fail("Pick a plan above first.", "plans");
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(contactEmail.trim()))
      return fail("Enter a valid contact email.", "company");
    if (!allAgreed) return fail("Please agree to all terms below before paying.", "terms");
    ...
    try {
      const signed = await fetch("/api/payway/sign", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amount, currency: "USD", email: contactEmail, payment_option,
          plan: plan.name, company: companyName, contact_name: contactName,
        }),
      }).then((r) => r.json());
      ...
      AbaPayway.checkout();
      setPayState("waiting");
      setPayMsg("Complete the payment in the ABA window…");
  ...
  return (
    <main className="min-h-screen bg-yai-bg text-yai-navy">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 sm:p-6 max-w-md shadow-sm">
          ...
          <div className="pb-4 border-b border-gray-100">
            ...
            <div className="font-semibold text-yai-navy mt-0.5">
              {plan ? `${plan.name} · $${amount.toLocaleString()}` : "Select a plan above"}
            </div>
          </div>

          {/* Method rows — mirror ABA's official checkout chooser */}
          <div className="mt-4 space-y-3">
            <PayMethodRow
              onClick={() => payWithAba("abapay_khqr")}
              disabled={busy}
              title="ABA KHQR"
          ...
          {payMsg && (
            <div className={`mt-3 text-[13px] rounded-xl p-3 ${
              payState === "failed" ? "bg-red-50 border border-red-200 text-red-700"
      ...
      <div className="max-w-6xl mx-auto px-6 mt-8 mb-16 text-sm text-gray-600">
        ...
        {plan && <span className="ml-2 text-yai-navy">· ${amount.toLocaleString()}</span>}
...
function PaymentSuccess({
  plan, amount, email, company, tranId,
}: {
  plan: string | null; amount: number; email: string; company: string; tranId: string;
}) {
  return (
    <main className="min-h-screen bg-yai-bg text-yai-navy flex flex-col">
      <div className="bg-yai-navy text-white">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-lg bg-white rounded-2xl border border-gray-200 shadow-sm p-8 sm:p-10 text-center">
          ...
          <p className="text-gray-600 mt-3 leading-relaxed">
            Your payment has been confirmed. A member of our team will contact you shortly at your registered
            email to finalise onboarding and provision your workspace.
          </p>
          ...
          <div className="mt-6 rounded-xl border border-gray-100 bg-gray-50 p-4 text-left text-sm divide-y divide-gray-100">
            ...
            <Row label="Amount paid" value={`$${amount.toLocaleString()} USD`} />
          ...
          <p className="text-[12px] text-gray-400 mt-5">
            Keep your reference number for your records. Didn&apos;t receive an email within 24 hours?
            Contact <a href="mailto:gamini@yaikh.com" className="text-yai-navy underline">gamini@yaikh.com</a>.
          </p>
