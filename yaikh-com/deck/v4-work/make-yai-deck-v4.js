...
<table class="plan">
  ...
  <tbody>
    ${CUSTOMERS.map(([c, amt]) => `<tr><td class="who">${esc(c) || "&nbsp;"}</td>${cells.map(() => `<td>${amt ? esc(amt) : ""}</td>`).join("")}</tr>`).join("")}
  </tbody>
</table>
`, { cls: "planslide" }));
}
...
.planslide h2{font-size:34px;margin-bottom:14px}.plan{width:100%;border-collapse:collapse;table-layout:fixed;margin-top:0}.plan th,.plan td{border:1px solid rgba(255,255,255,.16);height:38px}.plan th{font-size:17px;font-weight:700;color:var(--gold);letter-spacing:.04em;padding:4px}.plan th.yr{background:rgba(243,112,33,.22);color:#fff;font-size:19px}.plan th.who,.plan td.who{width:210px;text-align:left;padding-left:12px;font-size:19px;color:#fff}.plan td{background:rgba(255,255,255,.04);text-align:center;font-size:15px;color:#DCE4F5}
