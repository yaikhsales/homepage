...
if (TIER.client) {
  S.splice(1);
  n = 1;
  const sec = (x) => { x.priced = x.g.some((g) => g && (Array.isArray(g[0]) ? g.some((h) => h[2]) : g[2])); return `<div class="csec ${x.c}"><div class="csec-h">${esc(x.s)}</div><div class="ctabs" style="grid-template-columns:repeat(${x.cols},1fr)">${x.g.map((g) => { const one = ([t, m, price]) => `${t ? `${price ? `<span class="cprice">${esc(price)}</span>` : x.priced ? `<span class="cprice ghost">&nbsp;</span>` : ""}<b>${esc(t)}</b>` : `${x.priced ? `<span class="cprice ghost">&nbsp;</span>` : ""}<b style="visibility:hidden">&nbsp;</b>`}<ul>${m.map((a) => `<li>${esc(a)}</li>`).join("")}</ul>`; return !g ? `<div class="ctab"></div>` : Array.isArray(g[0]) ? `<div class="ctab stack">${g.map(one).join("")}</div>` : `<div class="ctab">${one(g)}</div>`; }).join("")}</div></div>`; }
...
.constslide h2{font-size:36px;margin-bottom:16px}.const{display:grid;grid-template-columns:10.5fr 3fr 6.5fr;gap:12px;align-items:stretch}.crow2{display:grid;grid-template-columns:3fr 5fr;gap:14px}.csec{border-radius:12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);overflow:hidden}.csec-h{display:flex;align-items:center;justify-content:center;text-align:center;height:58px;line-height:1.2;font-size:17px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;padding:0 10px;color:#fff}.csec.admin .csec-h{background:var(--blue)}.csec.mgmt .csec-h{background:#6D28D9}.csec.ops .csec-h{background:var(--green)}.ctabs{display:grid;gap:0}.ctab{padding:10px 10px 12px;border-top:1px solid rgba(255,255,255,.08)}.ctab b{display:block;font-size:21px;color:var(--gold);margin-bottom:4px}.ctab ul{margin:0;padding:0;list-style:none}.cprice{display:block;font-size:19px;font-weight:800;color:#fff;background:rgba(255,213,138,.14);border:1px dashed rgba(255,213,138,.55);border-radius:6px;padding:1px 8px;margin:0 0 4px;width:max-content;min-width:78px}.cprice.ghost{visibility:hidden}.ctab.stack ul{margin-bottom:12px}.ctab li{font-size:19px;line-height:1.3;hyphens:none;overflow-wrap:normal;color:#E6ECFA}
