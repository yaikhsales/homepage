...
if (TIER.client) {
  S.splice(1);
  n = 1;
  const sec = (x) => `<div class="csec ${x.c}"><div class="csec-h">${esc(x.s)}</div><div class="ctabs" style="grid-template-columns:repeat(${x.cols},1fr)">${x.g.map(([t, m]) => `<div class="ctab"><b>${esc(t)}</b><ul>${m.map((a) => `<li>${esc(a)}</li>`).join("")}</ul></div>`).join("")}</div></div>`;
...
.constslide h2{font-size:36px;margin-bottom:16px}.const{display:grid;grid-template-columns:10.5fr 3fr 6.5fr;gap:12px;align-items:stretch}.crow2{display:grid;grid-template-columns:3fr 5fr;gap:14px}.csec{border-radius:12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);overflow:hidden}.csec-h{font-size:17px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;padding:7px 14px;color:#fff}.csec.admin .csec-h{background:var(--blue)}.csec.mgmt .csec-h{background:#6D28D9}.csec.ops .csec-h{background:var(--green)}.ctabs{display:grid;gap:0}.ctab{padding:10px 10px 12px;border-top:1px solid rgba(255,255,255,.08)}.ctab b{display:block;font-size:21px;color:var(--gold);margin-bottom:4px}.ctab ul{margin:0;padding:0;list-style:none}.ctab li{font-size:19px;line-height:1.3;hyphens:none;overflow-wrap:normal;color:#E6ECFA}
