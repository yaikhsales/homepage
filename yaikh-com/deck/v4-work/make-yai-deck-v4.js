...
<table class="plan">
  ...
  <tbody>
    ${CUSTOMERS.map((c) => `<tr><td class="who">${c.n ? esc(c.n) : "&nbsp;"}${c.note ? ` <span class="pnote">· ${esc(c.note)}</span>` : ""}</td>${cells.map(([, key]) => { const v = (c.at && c.at[key]) || c.all || ""; const isPaid = c.paid && c.paid.includes(key); const setting = key.endsWith("2026"); return `<td${v ? ` class="${isPaid ? "paid" : setting && !c.all ? "setting" : "hit"}"` : ""}>${esc(v)}${!v ? "" : isPaid ? `<span class="paidtag">Paid</span>` : c.all || !setting ? "" : `<span class="settag">Setting</span>`}</td>`; }).join("")}</tr>`).join("")}
  </tbody>
</table>
`, { cls: "planslide" }));
}
...
.planslide h2{font-size:34px;margin-bottom:14px}.plan{width:100%;border-collapse:collapse;table-layout:fixed;margin-top:0}.plan th,.plan td{border:1px solid rgba(255,255,255,.16);height:38px}.plan th{font-size:17px;font-weight:700;color:var(--gold);letter-spacing:.04em;padding:4px}.plan th.yr{background:rgba(243,112,33,.22);color:#fff;font-size:19px;text-align:center}.pnote{font-size:15px;color:#8FA8D8;font-weight:400}.planslide{padding-left:40px;padding-right:40px}.plan td.hit{background:rgba(243,112,33,.28);color:#fff;font-weight:700}.plan td.paid{background:#BBF7D0;color:#065F46;font-weight:800}.plan td.setting{background:#FED7AA;color:#9A3412;font-weight:800}.settag{display:block;font-size:11px;font-weight:800;color:#9A3412;letter-spacing:.06em;text-transform:uppercase;margin-top:-2px}.paidtag{display:block;font-size:11px;font-weight:800;color:#065F46;letter-spacing:.06em;text-transform:uppercase;margin-top:-2px}.plan th.who,.plan td.who{width:246px;text-align:left;padding-left:8px;font-size:18px;color:#fff}.plan td{background:rgba(255,255,255,.04);text-align:center;font-size:15px;color:#DCE4F5;overflow:hidden;padding:7px 2px}.plan td.paid,.plan td.setting{font-size:14px;line-height:1.15}
