...
function summaryFor(pa: string, pill: string, workers: number, lines: number, product: string): string {
  ...
  const map: Record<string, () => string> = {
    ...
    "Capacity plan":             () => `Capacity utilisation ${rand(65,92)}% this month across ${lines} lines. ${pick(["Line B has 12% headroom next week","All lines saturated through ETD","Cutting is the bottleneck, sewing has room"])}.`,
    "Factory plan":              () => `Factory plan for ${dayName(rand(1,7))} — ${rand(3800,4600)} pcs across all lines. ${pick(["On track","Behind by 6%","Ahead by 4%"])}.`,
    "Line plan":                 () => `Line ${pick(["A","B","C"])} plan for tomorrow: PO-${rand(2026,2027)}-${rand(30,80)} · ${rand(600,1400)} pcs · standard time ${rand(11,25)} min. Skill mix ${pick(["balanced","short 2 overlock operators","well-covered"])}.`,
    "Sales situation":           () => `${rand(3,8)} POs in pipeline for next 30 days · total ${rand(120,320)}k pcs · biggest buyer ${pick(["BuyerCo","TargetCo","EU-Direct"])}. ${pick(["Steady","One PO at risk of push-out","New enquiry from Uniqlo pending decision"])}.`,
    "Planning situation":        () => `Plan vs actual last 7 days: output ${rand(88,105)}% of plan. ${pick(["Fabric shortage on Line B slowed 2 days","Machine downtime cost ~1,200 pcs","No blockers"])}. Needs ${pick(["boss sign-off on OT","re-sequence for Uniqlo PO","supplier follow-up"])}.`,
