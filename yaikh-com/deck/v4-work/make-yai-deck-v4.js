...
if (TIER.client) {
  S.splice(1);
  n = 1;
  const sec = (x) => `<div class="csec ${x.c}"><div class="csec-h">${esc(x.s)}</div><div class="ctabs" style="grid-template-columns:repeat(${x.cols},1fr)">${x.g.map((g) => { const one = ([t, m, price]) => `${t ? `<b>${esc(t)}</b><span class="cprice${price ? "" : " empty"}">${price ? esc(price) : "$ —"}</span>` : ""}<ul>${m.map((a) => `<li>${esc(a)}</li>`).join("")}</ul>`; return !g ? `<div class="ctab"></div>` : Array.isArray(g[0]) ? `<div class="ctab stack">${g.map(one).join("")}</div>` : `<div class="ctab">${one(g)}</div>`; }).join("")}</div></div>`;
