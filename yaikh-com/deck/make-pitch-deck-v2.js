...
/* ---------- 5b · OUR AGENTS (mirrors yaikh.com/experience layout) ---------- */
{
  ...
  // Header row — Yai logo + "Yai Agents" title, right-side kicker
  addRoundLogo(s, 0.35, 0.15, 0.8);
  s.addText("Yai Agents", {
    x: 1.25, y: 0.2, w: 3.5, h: 0.7,
    fontFace: F, fontSize: 26, bold: true, color: "FFFFFF",
    valign: "middle", margin: 0,
  });
  s.addText("05 / OUR AGENTS", {
  ...
  const cats = [
    {
      title: "Administration", bandColor: BLUE, subBg: "1E3A8A",
      x: admX, w: admW,
      subs: [
        { name: "Accountant", tiles: ["Accountant", "IEWS"] },
        { name: "Billing",    tiles: ["Purchase Req.", "Bill Claim", "Salary Bill", "Shipping Bill"] },
        { name: "HR",         tiles: ["YHR", "Org Chart", "Training", "Temp. Worker", "Speak Up"] },
        { name: "Admin",      tiles: ["Support Ticket", "Y Shop", "Gate Pass", "Meeting Room", "Car Booking", "Fire Alarm", "CCTV"] },
        { name: "CSR",        tiles: ["Digital Audit", "Energy", "Air", "Water", "Waste", "Chemical"] },
        { name: "Shipping",   tiles: ["Shipping"] },
        { name: "E-GOV",      tiles: ["E-Government"] },
      ],
    },
    {
      title: "Management Ai", bandColor: DARKGREEN, subBg: "0F3C2A",
      x: mgmX, w: mgmW,
      subs: [
        { name: "Dashboard",      tiles: ["Mgmt Dashboard", "SOP"] },
        { name: "Data Scientist", tiles: ["System Analysis"] },
      ],
    },
    {
      title: "Operations", bandColor: GREEN, subBg: "064E3B",
      x: opsX, w: opsW,
      subs: [
        { name: "QA",         tiles: ["YQMS", "Call Out"] },
        { name: "Production", tiles: ["FC", "YWIP", "CE", "YTM", "YTM Shop"] },
        { name: "4DP",        tiles: ["4DP"] },
        { name: "YPI",        tiles: ["YPI"] },
        { name: "MRP",        tiles: ["MRP"] },
      ],
...
{
  const s = pres.addSlide();
  titleBlock(s, "06 / Pricing & packaging", "Start $120 · scale as the business grows.");
...
{
  const s = pres.addSlide();
  titleBlock(s, "08 / ASEAN approach", "");
...
{
  const s = pres.addSlide();
  titleBlock(s, "09 / Partners & Customers", "");
...
{
  const s = pres.addSlide();
  titleBlock(s, "10 / Startup support needed", "Three levers · both sides win.");
